...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    float *host_output_nonconst = const_cast<float *>(host_output); // This is a hack to avoid having to change the function signature to non-const
    ...
    for (int i = 0; i < B; i += seg_size * NUM_STREAMS) {
        for (int j = 0; j < NUM_STREAMS && i + j * seg_size < B; j++) {
            ...
            cudaMemcpyAsync(*device_input_ptr + (i + j * seg_size) * C * H * W, host_input + (i + j * seg_size) * C * H * W, size * C * H * W * sizeof(float), cudaMemcpyHostToDevice, streams[j]);
        }
        ...
        for (int j = 0; j < NUM_STREAMS && i + j * seg_size < B; j++) {
            ...
            if (M < 16) {
                ...
                mat_forward_kernel_tensor_8_32_16<<<dimGrid, dimBlock, 0, streams[j]>>>(*device_output_ptr + (i + j * seg_size) * M * H_out * W_out, *device_input_ptr + (i + j * seg_size) * C * H * W, *device_mask_ptr, size, M, C, H, W, K, S);
            } else {
                dim3 dimBlock = dim3(16, 2, 1);
                dim3 dimGrid = dim3(ceil((H_out*W_out)/(float)16), ceil(M/(float)16), size);
                mat_forward_kernel_tensor<<<dimGrid, dimBlock, 0, streams[j]>>>(*device_output_ptr + (i + j * seg_size) * M * H_out * W_out, *device_input_ptr + (i + j * seg_size) * C * H * W, *device_mask_ptr, size, M, C, H, W, K, S);
            }
        }

        for (int j = 0; j < NUM_STREAMS && i + j * seg_size < B; j++) {
            int size = min(seg_size, B - i - j * seg_size);
            cudaMemcpyAsync(host_output_nonconst + (i + j * seg_size) * M * H_out * W_out, *device_output_ptr + (i + j * seg_size) * M * H_out * W_out, size * M * H_out * W_out * sizeof(float), cudaMemcpyDeviceToHost, streams[j]);
        }
    }
}
