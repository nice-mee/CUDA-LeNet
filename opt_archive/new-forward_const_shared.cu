...
__global__ void unroll_kernel(const float *X, float *X_unroll, const int C , const int K, const int H, const int W, const int S)
{
    int H_out = (H - K)/S + 1;
    int W_out = (W - K)/S + 1;
    int b = blockIdx.x;
    int c = blockIdx.y;
    int w_base = c * K * K;
    int h = (blockIdx.z * 1024 + threadIdx.x) / W_out;
    int w = (blockIdx.z * 1024 + threadIdx.x) % W_out;
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    dim3 dimGrid(B, C, ceil((H_out*W_out)/1024.0));
    unroll_kernel<<<dimGrid, dimBlock>>>(device_input, device_unroll, C, K, H, W, S);
