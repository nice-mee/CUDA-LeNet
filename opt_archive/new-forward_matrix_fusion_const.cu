...
__constant__ float mask_const[8000];

__global__ void mat_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    for (int i = 0; i < ceil(numAColumns/(float) TILE_WIDTH); i++) {
        ...
        if (Row < M && ColA < numAColumns)
            tileA[threadIdx.y][threadIdx.x] = mask_const[Row * numAColumns + ColA];
        else
