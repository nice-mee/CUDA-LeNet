...
__global__ void mat_forward_kernel_tensor(half *output, const half *input, const half *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    __shared__ half tileB[2][16][16];
    __shared__ half tileC[2][16][16];
    ...
    for (int i = 0; i < ceil(numAColumns/(float) 16); i++) {
        ...
        int q = RowB % K;
        for (int j = 0; j < 8; j++) {
            int ColB = blockIdx.x * 32 + threadIdx.y * 8 + j;
            int h = ColB / W_out;
            int w = ColB % W_out;
            if (tile_idx == 0) {
                int RowA = blockIdx.y * 16 + threadIdx.y * 8 + j;
                if (RowA < M && ColA < numAColumns)
                    tileA[threadIdx.y * 8 + j][threadIdx.x] = mask[RowA * numAColumns + ColA];
                else
                    tileA[threadIdx.y * 8 + j][threadIdx.x] = 0.0;
            }
            if (ColB < numBColumns && RowB < numAColumns)
                tileB[tile_idx][threadIdx.x][(threadIdx.y % 2) * 8 + j] = in_4d(b, c, h * S + p, w * S + q);
            else
                tileB[tile_idx][threadIdx.x][(threadIdx.y % 2) * 8 + j] = 0.0;
        }
        __syncthreads();

        wmma::load_matrix_sync(a_frag, (half *) tileA, 16);
        wmma::load_matrix_sync(b_frag, (half *) &tileB[tile_idx][0][0], 16);
        wmma::mma_sync(c_frag, a_frag, b_frag, c_frag);
    }
    wmma::store_matrix_sync((half *) &tileC[tile_idx][0][0], c_frag, 16, wmma::mem_row_major);
    __syncthreads();
    for (int i = 0; i < 8; i++) {
        int RowC = blockIdx.y * 16 + (threadIdx.y % 2) * 8 + i;
        int ColC = blockIdx.x * 32 + tile_idx * 16 + threadIdx.x;
        if (RowC < M && ColC < numBColumns)
            output[b * (M * numBColumns) + RowC * numBColumns + ColC] = tileC[tile_idx][(threadIdx.y % 2) * 8 + i][threadIdx.x];
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    dim3 dimBlock = dim3(16, 4, 1);
    dim3 dimGrid = dim3(ceil((H_out*W_out)/(float)32), ceil(M/(float)16), B);
