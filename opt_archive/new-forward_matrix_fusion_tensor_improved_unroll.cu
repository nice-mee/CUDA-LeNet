...
/* This optimization is not really useful */
...
__global__ void mat_forward_kernel_tensor(half * __restrict__ output, const half * __restrict__ input, const half * __restrict__ mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    for (int i = 0; i < ceil(numAColumns/(float) TILE_WIDTH); i++) {
        ...
        int RowB = i * TILE_WIDTH + threadIdx.x;
        int c = RowB / (K*K);
        int p = RowB % (K*K) / K;   
        int q = RowB % K;
        #pragma unroll
        for (int j = 0; j < 8; j++) {
            ...
            if (ColB < numBColumns && RowB < numAColumns)
                tileB[threadIdx.x][threadIdx.y * 8 + j] = in_4d(b, c, h * S + p, w * S + q);
            else
                tileB[threadIdx.x][threadIdx.y * 8 + j] = 0.0;
        }
        __syncthreads();
